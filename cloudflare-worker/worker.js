...
async function addRequest(request,env,core){const body=await request.json().catch(()=>({}));const raw=String(body.text||body.song||body.request||"").trim();if(!raw)return json({ok:false,error:"Missing request text"},400);const item=normalizeRequest({...body,user:body.user||body.username||body.displayName||"Twitch chat",song:cleanRequestSong(raw),language:body.language||detectRequestLanguage(raw),show:body.show||core.activeTheme||"DJ FOLSOE LIVE",status:body.status||"approved",time:body.time||new Date().toISOString()},0);const current=await getRequestList(env,core);const list=await putRequestList(env,core,[item,...current]);return json({ok:true,item,items:list.slice(0,Number(core.requestSettings?.visible||3)),all:list,stats:requestStats(list)});}
