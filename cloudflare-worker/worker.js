...
function safeTwitchLogin(value, env, core){
  let raw = value;
  if(raw && typeof raw === "object"){
    raw = raw.login || raw.channel || raw.name || raw.twitchChannel || "";
  }
  raw = String(raw || core?.twitchChannel || env?.TWITCH_CHANNEL || "djfolsoe").trim().toLowerCase();
  if(raw === "[object object]" || raw === "undefined" || raw === "null" || !raw) raw = "djfolsoe";
  raw = raw.replace(/^@/,"").replace(/^https?:\/\/(www\.)?twitch\.tv\//,"").replace(/[^a-z0-9_]/g,"");
  return raw || "djfolsoe";
}
...
async function twitchProfile(env, core, loginOverride) {
  const login = safeTwitchLogin(loginOverride, env, core);
  const fallback = {
...
async function homepage(env, core) {
  ...
  return {
    ...
    shows:(core.shows&&core.shows.length?core.shows:DEFAULT_CORE.shows||[]),
    showVisuals:core.showVisuals||DEFAULT_CORE.showVisuals||{},
    showVisuals:core.showVisuals||DEFAULT_CORE.showVisuals||{},
    newsCards:sortItems(core.homepageNews||DEFAULT_CORE.homepageNews),
    top20:(core.top20&&core.top20.length?core.top20:DEFAULT_CORE.top20||[]).slice(0,20),
    ...
    comingUp:v820Next(core,[{"id": "show_001", "date": "2026-08-14", "start": "20:00", "end": "23:00", "title": "Summer Closing Party", "show": "Fredagsbar", "theme": "SUMMER", "type": "Live DJ Show", "description": "Sommerens sidste store fest med live DJ, requests og fællesskab.", "cover": "", "featured": true, "liveEvent": false, "specialEvent": true, "active": true, "priority": 1}, {"id": "show_002", "date": "2026-08-18", "start": "20:00", "end": "23:00", "title": "Trance Tuesday", "show": "Trance Tuesday", "theme": "TRANCE", "type": "Trance", "description": "Melodisk trance, energi og store følelser.", "cover": "", "featured": false, "liveEvent": false, "specialEvent": false, "active": true, "priority": 2}, {"id": "show_003", "date": "2026-08-21", "start": "20:00", "end": "23:30", "title": "Fredagsbar", "show": "Fredagsbar", "theme": "FREDAGSBAR", "type": "Party", "description": "Weekendstemning, sjov og ballade direkte fra Danmark.", "cover": "", "featured": false, "liveEvent": false, "specialEvent": false, "active": true, "priority": 3}]).slice(1,5),
    discoveryUniverse:v820Items(core,"discoveryUniverse",[{"category": "Discovery", "artist": "Mau P", "title": "The Less I Know The Better", "note": "Ny energi til chart-showet.", "active": true, "priority": 1}, {"category": "Bubbling Under", "artist": "Peggy Gou", "title": "Find The Way", "note": "Frisk house-vibe til streamen.", "active": true, "priority": 2}, {"category": "Future Hits", "artist": "Anyma", "title": "Hypnotized", "note": "Stærk kandidat til kommende shows.", "active": true, "priority": 3}, {"category": "DJ Picks", "artist": "DJ FOLSOE", "title": "Viewer Pick", "note": "Plads til dine egne opdagelser.", "active": true, "priority": 4}]),
    hallOfFameV820:v820Items(core,"hallOfFameV820",[{"title": "Månedens chatter", "name": "Twitch chatten", "text": "Den der holder energien oppe.", "icon": "💬", "active": true, "priority": 1}, {"title": "Månedens supporter", "name": "Community Hero", "text": "Kærlighed til dem der støtter udviklingen.", "icon": "💜", "active": true, "priority": 2}, {"title": "Månedens request", "name": "Top Requester", "text": "Den der finder de bedste tracks.", "icon": "🎧", "active": true, "priority": 3}, {"title": "Månedens raid", "name": "Raid Love", "text": "Når fællesskabet vokser på tværs.", "icon": "🚀", "active": true, "priority": 4}, {"title": "Månedens discovery", "name": "Future Hit", "text": "Det nye track vi ikke kan slippe.", "icon": "🏆", "active": true, "priority": 5}]),
    requestStats:reqPayload?.stats||{},
    mods:await enrichMods(env, core),
    communityWall:core.communityWall||DEFAULT_CORE.communityWall||[],
...
function v81613OverlayItems(core, key, fallbackItems){
  const custom = core.overlayContent && Array.isArray(core.overlayContent[key]) ? core.overlayContent[key] : [];
  const usable = custom.filter(x=>x && x.active !== false).sort((a,b)=>Number(a.priority||99)-Number(b.priority||99));
  return usable.length ? usable.map(x=>({label:x.label, headline:x.headline, body:x.body, icon:x.icon||"•"})) : fallbackItems;
}
...
async function twitchChannelFull(env, core){
  const login = safeTwitchLogin(core.twitchChannel, env, core);
  const fallback = {
    ok:false, login, displayName:"DJ FOLSOE", avatar:core.profile?.fallbackAvatar||"",
...
async function saveDiscoveryPicks(request, env, core){
  if (!adminOk(request, env)) return json({error:"Unauthorized"},401);
  const body = await request.json().catch(()=>({}));
  const items = Array.isArray(body.items) ? body.items : (Array.isArray(body.discoveryPicks) ? body.discoveryPicks : []);
  core.discoveryPicks = items.slice(0,3).map((x,i)=>({
  ...
  await putCore(env, core);
  return json({ok:true,items:core.discoveryPicks});
}


async function enrichMods(env, core){
  const mods = (core.modTeam || core.profile?.mods || DEFAULT_CORE.modTeam || []).filter(x=>x && x.active !== false).sort((a,b)=>Number(a.priority||99)-Number(b.priority||99));
  const enriched = [];
  for(const m of mods){
    const login = safeTwitchLogin(m.login || m.name || m.twitch || "", env, core);
    let tw = {};
    try{ tw = await twitchProfile(env, {...core,twitchChannel:login}); }catch(e){ tw = {}; }
    enriched.push({
      ...m,
      login,
      displayName:tw.displayName || m.name || login,
      avatar:tw.avatar || "",
      isLive:!!tw.isLive,
      viewers:tw.viewers || 0,
      twitchUrl:"https://twitch.tv/"+login
    });
  }
  return enriched;
}
async function saveObject(request, env, core, key){
  if (!adminOk(request,env)) return json({error:"Unauthorized"},401);
  const body = await request.json().catch(()=>({}));
  core[key] = body.item || body.items || body[key] || body;
  await putCore(env,core);
  return json({ok:true,item:core[key],items:core[key]});
}


function seoPick(obj, lang, fallback=""){ if(!obj)return fallback; if(typeof obj==="string")return obj; return obj[lang]||obj.da||obj.en||fallback; }
function buildSeoPayload(core, lang="da"){
  const seo=core.seo||DEFAULT_CORE.seo||{};
  const domain=(seo.domain||"https://folsoetv.dk").replace(/\/$/,"");
  const title=seoPick(seo.title,lang,"DJ FOLSOE | Danish Twitch DJ");
  const description=seoPick(seo.description,lang,"Danish Twitch DJ and music streamer.");
  const image=seo.image||`${domain}/assets/og-dj-folsoe.jpg`;
  const sameAs=Array.isArray(seo.sameAs)?seo.sameAs:["https://twitch.tv/djfolsoe"];
  const shows=Array.isArray(seo.showPages)?seo.showPages:[];
  const schema={"@context":"https://schema.org","@graph":[
    {"@type":"Person","@id":`${domain}/#person`,"name":"DJ FOLSOE","alternateName":["DJ Folsoe","DJFOLSOE","Sune Folsø"],"url":domain,"image":image,"sameAs":sameAs,"jobTitle":"Twitch DJ and Music Streamer","nationality":"Denmark","knowsAbout":["Dance music","Trance","Eurodance","Retro Hits","Twitch streaming","DJ livestreams"]},
    {"@type":"WebSite","@id":`${domain}/#website`,"name":seo.siteName||"DJ FOLSOE TV","url":domain,"inLanguage":["da","en","de"],"description":description,"publisher":{"@id":`${domain}/#person`}},
    {"@type":"Organization","@id":`${domain}/#organization`,"name":"FOLSOE TV","url":domain,"sameAs":sameAs}
  ]};
  shows.forEach(show=>schema["@graph"].push({"@type":"BroadcastEvent","name":show.title,"description":show.description,"isAccessibleForFree":true,"url":`${domain}/${show.slug}`,"broadcastOfEvent":{"@type":"MusicEvent","name":show.title},"organizer":{"@id":`${domain}/#person`}}));
  return {ok:true,version:VERSION,lang,title,description,keywords:seo.keywords||[],image,domain,sameAs,showPages:shows,schema};
}
function sitemapXml(core){
  const seo=core.seo||DEFAULT_CORE.seo||{}; const domain=(seo.domain||"https://folsoetv.dk").replace(/\/$/,"");
  const shows=Array.isArray(seo.showPages)?seo.showPages:[]; const pages=["",...shows.map(s=>s.slug)];
  const now=new Date().toISOString().slice(0,10);
  return `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${pages.map(p=>`  <url><loc>${domain}/${p}</loc><lastmod>${now}</lastmod><changefreq>weekly</changefreq><priority>${p?"0.8":"1.0"}</priority></url>`).join("\n")}\n</urlset>`;
}
function robotsTxt(core){ const seo=core.seo||DEFAULT_CORE.seo||{}; const domain=(seo.domain||"https://folsoetv.dk").replace(/\/$/,""); return `User-agent: *\nAllow: /\nSitemap: ${domain}/sitemap.xml\n`; }


async function saveDiscoveryPicksStable(request, env, core){
  if (!adminOk(request,env)) return json({error:"Unauthorized"},401);
  const body = await request.json().catch(()=>({}));
...
async function saveTop20Stable(request, env, core){
  if (!adminOk(request,env)) return json({error:"Unauthorized"},401);
  const body = await request.json().catch(()=>({}));
  const items = Array.isArray(body.items) ? body.items : [];
  core.top20 = items.slice(0,20).map((x,i)=>({
  ...
  await putCore(env,core);
  return json({ok:true,items:core.top20});
}


function detectRequestLanguage(text){const s=String(text||"").trim().toLowerCase();if(s.startsWith("!wunsch"))return"de";if(s.startsWith("!request"))return"en";return"da";}
function cleanRequestSong(text){return String(text||"").trim().replace(/^!(ønske|onske|request|wunsch)\s*/i,"").trim();}
function requestArtist(song){const s=String(song||"");const split=s.includes(" - ")?" - ":(s.includes(" – ")?" – ":"");return split?s.split(split)[0].trim():"";}
function normalizeRequest(x,i){const raw=x.song||x.text||x.request||"";return{id:String(x.id||("req_"+Date.now()+"_"+i+"_"+Math.random().toString(36).slice(2,7))),user:String(x.user||x.username||x.name||"Twitch chat").trim(),song:String(raw).trim(),language:String(x.language||detectRequestLanguage(raw)).trim(),show:String(x.show||"DJ FOLSOE LIVE").trim(),status:String(x.status||"approved").trim(),pinned:!!x.pinned,time:x.time||x.createdAt||new Date().toISOString(),priority:Number(x.priority||i+1)};}
async function getRequestList(env,core){try{if(env.DJF_DATA&&env.DJF_DATA.get){const saved=await env.DJF_DATA.get("song-requests","json");if(Array.isArray(saved))return saved;}}catch(e){}return Array.isArray(core.requests)?core.requests:(DEFAULT_CORE.requests||[]);}
async function putRequestList(env,core,list){const keep=Number(core.requestSettings?.keep||100);const cleaned=(Array.isArray(list)?list:[]).map(normalizeRequest).slice(0,keep);if(env.DJF_DATA&&env.DJF_DATA.put)await env.DJF_DATA.put("song-requests",JSON.stringify(cleaned));core.requests=cleaned;try{await putCore(env,core);}catch(e){}return cleaned;}
function requestStats(list){const approved=(Array.isArray(list)?list:[]).filter(x=>x&&x.status!=="rejected");const todayKey=new Date().toISOString().slice(0,10);const today=approved.filter(x=>String(x.time||"").slice(0,10)===todayKey);const artists={},users={},langs={},shows={};approved.forEach(x=>{const a=requestArtist(x.song||"");if(a)artists[a]=(artists[a]||0)+1;if(x.user)users[x.user]=(users[x.user]||0)+1;if(x.language)langs[x.language]=(langs[x.language]||0)+1;if(x.show)shows[x.show]=(shows[x.show]||0)+1;});const top=o=>Object.entries(o).sort((a,b)=>b[1]-a[1])[0]||["-",0];const ta=top(artists),tu=top(users),ts=top(shows);return{total:approved.length,today:today.length,topArtist:{name:ta[0],count:ta[1]},topRequester:{name:tu[0],count:tu[1]},topShow:{name:ts[0],count:ts[1]},languages:langs};}
async function recentRequestsPayload(env,core){const list=(await getRequestList(env,core)).map(normalizeRequest);const approved=list.filter(x=>x.status!=="rejected");const pinned=approved.filter(x=>x.pinned).sort((a,b)=>Number(a.priority||99)-Number(b.priority||99));const normal=approved.filter(x=>!x.pinned);const visible=Number(core.requestSettings?.visible||3);return{ok:true,items:[...pinned,...normal].slice(0,visible),all:list,stats:requestStats(list)};}
async function updateRequests(request,env,core){if(!adminOk(request,env))return json({error:"Unauthorized"},401);const body=await request.json().catch(()=>({}));const items=Array.isArray(body.items)?body.items.map(normalizeRequest):[];const list=await putRequestList(env,core,items);return json({ok:true,items:list,all:list,stats:requestStats(list)});}


async function saveCEList(request, env, core, key){
  if (!adminOk(request,env)) return json({error:"Unauthorized"},401);
  const body = await request.json().catch(()=>({}));
  core[key] = Array.isArray(body.items) ? body.items : [];
  await putCore(env,core);
...
function ceItems(core,key,fallback){
  const items = Array.isArray(core[key]) && core[key].length ? core[key] : fallback;
  return items.filter(x=>x && x.active !== false).sort((a,b)=>Number(a.priority||99)-Number(b.priority||99));
}


// ===== V816.20 TV Station Experience =====
function v820Items(core,key,fallback){const items=Array.isArray(core[key])&&core[key].length?core[key]:fallback;return items.filter(x=>x&&x.active!==false).sort((a,b)=>Number(a.priority||99)-Number(b.priority||99));}
function v820Ms(x){const iso=(x.date||'')+'T'+(x.start||'00:00')+':00';const ms=Date.parse(iso);return Number.isFinite(ms)?ms:9999999999999;}
function v820Next(core,fallback){const now=Date.now()-28800000;return v820Items(core,'broadcastPlanner',fallback).filter(x=>v820Ms(x)>=now).sort((a,b)=>v820Ms(a)-v820Ms(b));}
async function v820Save(request,env,core,key){if(!adminOk(request,env))return json({error:'Unauthorized'},401);const body=await request.json().catch(()=>({}));core[key]=Array.isArray(body.items)?body.items:[];await putCore(env,core);return json({ok:true,items:core[key]});}


// ===== V816.21 Homepage Broadcast Director =====
function v821Layout(core){
  const fallback=[{"key": "hero", "label": "Hero", "selector": "#hero", "visible": true, "priority": 1, "featured": false, "breaking": false}, {"key": "nextshow", "label": "Næste show", "selector": "#nextshow", "visible": true, "priority": 2, "featured": true, "breaking": false}, {"key": "who", "label": "Hvem er DJ FOLSOE", "selector": "#about,#who,#hvem", "visible": true, "priority": 3, "featured": false, "breaking": false}, {"key": "mods", "label": "Mod Team", "selector": "#mods,#modteam,#modTeam", "visible": true, "priority": 4, "featured": false, "breaking": false}, {"key": "shows", "label": "Shows", "selector": "#shows", "visible": true, "priority": 5, "featured": false, "breaking": false}, {"key": "top20", "label": "FOLSOE Top 20", "selector": "#top20", "visible": true, "priority": 6, "featured": false, "breaking": false}, {"key": "discoveryuniverse", "label": "Music Discovery", "selector": "#discoveryuniverse,#musicdiscovery,#musicDiscovery", "visible": true, "priority": 7, "featured": false, "breaking": false}, {"key": "livewall", "label": "Live Request Wall", "selector": "#livewall,#requests", "visible": true, "priority": 8, "featured": false, "breaking": false}, {"key": "community", "label": "Community", "selector": "#community", "visible": true, "priority": 9, "featured": false, "breaking": false}, {"key": "viewerjourney", "label": "Follower Journey", "selector": "#viewerjourney,#viewerJourney", "visible": true, "priority": 10, "featured": false, "breaking": false}, {"key": "halloffame", "label": "Hall Of Fame", "selector": "#halloffame,#hallOfFame", "visible": true, "priority": 11, "featured": false, "breaking": false}, {"key": "djnetwork", "label": "DJ Network", "selector": "#djnetwork,#djNetwork", "visible": true, "priority": 12, "featured": false, "breaking": false}, {"key": "comingup", "label": "Coming Up", "selector": "#comingup", "visible": true, "priority": 13, "featured": false, "breaking": false}, {"key": "tvguide", "label": "TV Guide", "selector": "#tvguide", "visible": false, "priority": 80, "featured": false, "breaking": false}, {"key": "showarchive", "label": "Show Archive", "selector": "#showarchive", "visible": false, "priority": 90, "featured": false, "breaking": false}];
  const items=Array.isArray(core.homepageLayout)&&core.homepageLayout.length?core.homepageLayout:fallback;
  return items.sort((a,b)=>Number(a.priority||99)-Number(b.priority||99));
}
async function v821SaveLayout(request,env,core){
  if(!adminOk(request,env))return json({error:"Unauthorized"},401);
  const body=await request.json().catch(()=>({}));
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/api/health") return json({ok:true,version:VERSION,route:"worker-active",homepage:true,layout:true});
      ...
      if (path === "/api/homepage-layout") {
        if (request.method === "GET") return json({ok:true,items:v821LayoutHotfix(core)});
        return v821SaveLayoutHotfix(request,env,core);
      }
      if (path === "/api/tv-station") {
        const p=v820Items(core,"broadcastPlanner",[{"id": "show_001", "date": "2026-08-14", "start": "20:00", "end": "23:00", "title": "Summer Closing Party", "show": "Fredagsbar", "theme": "SUMMER", "type": "Live DJ Show", "description": "Sommerens sidste store fest med live DJ, requests og fællesskab.", "cover": "", "featured": true, "liveEvent": false, "specialEvent": true, "active": true, "priority": 1}, {"id": "show_002", "date": "2026-08-18", "start": "20:00", "end": "23:00", "title": "Trance Tuesday", "show": "Trance Tuesday", "theme": "TRANCE", "type": "Trance", "description": "Melodisk trance, energi og store følelser.", "cover": "", "featured": false, "liveEvent": false, "specialEvent": false, "active": true, "priority": 2}, {"id": "show_003", "date": "2026-08-21", "start": "20:00", "end": "23:30", "title": "Fredagsbar", "show": "Fredagsbar", "theme": "FREDAGSBAR", "type": "Party", "description": "Weekendstemning, sjov og ballade direkte fra Danmark.", "cover": "", "featured": false, "liveEvent": false, "specialEvent": false, "active": true, "priority": 3}]), n=v820Next(core,[{"id": "show_001", "date": "2026-08-14", "start": "20:00", "end": "23:00", "title": "Summer Closing Party", "show": "Fredagsbar", "theme": "SUMMER", "type": "Live DJ Show", "description": "Sommerens sidste store fest med live DJ, requests og fællesskab.", "cover": "", "featured": true, "liveEvent": false, "specialEvent": true, "active": true, "priority": 1}, {"id": "show_002", "date": "2026-08-18", "start": "20:00", "end": "23:00", "title": "Trance Tuesday", "show": "Trance Tuesday", "theme": "TRANCE", "type": "Trance", "description": "Melodisk trance, energi og store følelser.", "cover": "", "featured": false, "liveEvent": false, "specialEvent": false, "active": true, "priority": 2}, {"id": "show_003", "date": "2026-08-21", "start": "20:00", "end": "23:30", "title": "Fredagsbar", "show": "Fredagsbar", "theme": "FREDAGSBAR", "type": "Party", "description": "Weekendstemning, sjov og ballade direkte fra Danmark.", "cover": "", "featured": false, "liveEvent": false, "specialEvent": false, "active": true, "priority": 3}]);
        return json({ok:true,broadcastPlanner:p,nextShow:n[0]||p[0]||null,comingUp:n.slice(1,5),discoveryUniverse:v820Items(core,"discoveryUniverse",[{"category": "Discovery", "artist": "Mau P", "title": "The Less I Know The Better", "note": "Ny energi til chart-showet.", "active": true, "priority": 1}, {"category": "Bubbling Under", "artist": "Peggy Gou", "title": "Find The Way", "note": "Frisk house-vibe til streamen.", "active": true, "priority": 2}, {"category": "Future Hits", "artist": "Anyma", "title": "Hypnotized", "note": "Stærk kandidat til kommende shows.", "active": true, "priority": 3}, {"category": "DJ Picks", "artist": "DJ FOLSOE", "title": "Viewer Pick", "note": "Plads til dine egne opdagelser.", "active": true, "priority": 4}]),hallOfFameV820:v820Items(core,"hallOfFameV820",[{"title": "Månedens chatter", "name": "Twitch chatten", "text": "Den der holder energien oppe.", "icon": "💬", "active": true, "priority": 1}, {"title": "Månedens supporter", "name": "Community Hero", "text": "Kærlighed til dem der støtter udviklingen.", "icon": "💜", "active": true, "priority": 2}, {"title": "Månedens request", "name": "Top Requester", "text": "Den der finder de bedste tracks.", "icon": "🎧", "active": true, "priority": 3}, {"title": "Månedens raid", "name": "Raid Love", "text": "Når fællesskabet vokser på tværs.", "icon": "🚀", "active": true, "priority": 4}, {"title": "Månedens discovery", "name": "Future Hit", "text": "Det nye track vi ikke kan slippe.", "icon": "🏆", "active": true, "priority": 5}])});
      }
      if (path === "/api/broadcast-planner") { if (request.method==="GET") return json({ok:true,items:v820Items(core,"broadcastPlanner",[{"id": "show_001", "date": "2026-08-14", "start": "20:00", "end": "23:00", "title": "Summer Closing Party", "show": "Fredagsbar", "theme": "SUMMER", "type": "Live DJ Show", "description": "Sommerens sidste store fest med live DJ, requests og fællesskab.", "cover": "", "featured": true, "liveEvent": false, "specialEvent": true, "active": true, "priority": 1}, {"id": "show_002", "date": "2026-08-18", "start": "20:00", "end": "23:00", "title": "Trance Tuesday", "show": "Trance Tuesday", "theme": "TRANCE", "type": "Trance", "description": "Melodisk trance, energi og store følelser.", "cover": "", "featured": false, "liveEvent": false, "specialEvent": false, "active": true, "priority": 2}, {"id": "show_003", "date": "2026-08-21", "start": "20:00", "end": "23:30", "title": "Fredagsbar", "show": "Fredagsbar", "theme": "FREDAGSBAR", "type": "Party", "description": "Weekendstemning, sjov og ballade direkte fra Danmark.", "cover": "", "featured": false, "liveEvent": false, "specialEvent": false, "active": true, "priority": 3}])}); return v820Save(request,env,core,"broadcastPlanner"); }
      if (path === "/api/discovery-universe") { if (request.method==="GET") return json({ok:true,items:v820Items(core,"discoveryUniverse",[{"category": "Discovery", "artist": "Mau P", "title": "The Less I Know The Better", "note": "Ny energi til chart-showet.", "active": true, "priority": 1}, {"category": "Bubbling Under", "artist": "Peggy Gou", "title": "Find The Way", "note": "Frisk house-vibe til streamen.", "active": true, "priority": 2}, {"category": "Future Hits", "artist": "Anyma", "title": "Hypnotized", "note": "Stærk kandidat til kommende shows.", "active": true, "priority": 3}, {"category": "DJ Picks", "artist": "DJ FOLSOE", "title": "Viewer Pick", "note": "Plads til dine egne opdagelser.", "active": true, "priority": 4}])}); return v820Save(request,env,core,"discoveryUniverse"); }
      if (path === "/api/hall-of-fame-v820") { if (request.method==="GET") return json({ok:true,items:v820Items(core,"hallOfFameV820",[{"title": "Månedens chatter", "name": "Twitch chatten", "text": "Den der holder energien oppe.", "icon": "💬", "active": true, "priority": 1}, {"title": "Månedens supporter", "name": "Community Hero", "text": "Kærlighed til dem der støtter udviklingen.", "icon": "💜", "active": true, "priority": 2}, {"title": "Månedens request", "name": "Top Requester", "text": "Den der finder de bedste tracks.", "icon": "🎧", "active": true, "priority": 3}, {"title": "Månedens raid", "name": "Raid Love", "text": "Når fællesskabet vokser på tværs.", "icon": "🚀", "active": true, "priority": 4}, {"title": "Månedens discovery", "name": "Future Hit", "text": "Det nye track vi ikke kan slippe.", "icon": "🏆", "active": true, "priority": 5}])}); return v820Save(request,env,core,"hallOfFameV820"); }
      if (path === "/api/content-expansion") return json({ok:true,tvGuide:ceItems(core,"tvGuide",[{"day": "Tirsdag", "time": "18:30", "title": "Trance Tuesday", "text": "Store melodier, energi og trance-fællesskab.", "type": "trance", "active": true, "priority": 1}, {"day": "Torsdag", "time": "18:30", "title": "FOLSOE Top 20", "text": "Ugens største tracks i countdown-format.", "type": "chart", "active": true, "priority": 2}, {"day": "Fredag", "time": "20:00", "title": "Fredagsbar", "text": "Live DJ med sjov, ballade og weekendstemning.", "type": "party", "active": true, "priority": 3}, {"day": "Søndag", "time": "20:00", "title": "Retro Hits", "text": "Klassikere, nostalgi og gamle hits med nyt liv.", "type": "retro", "active": true, "priority": 4}]),viewerJourney:ceItems(core,"viewerJourney",[{"key": "followers", "label": "Followers", "current": 870, "target": 1000, "text": "Rejsen mod 1000 followers på Twitch.", "active": true, "priority": 1}, {"key": "subs", "label": "Subs", "current": 0, "target": 100, "text": "Subs hjælper med teknik, grafik og shows.", "active": true, "priority": 2}, {"key": "community", "label": "Community", "current": 0, "target": 500, "text": "Flere aktive seere og mere fællesskab.", "active": true, "priority": 3}]),hallOfFame:ceItems(core,"hallOfFame",[{"title": "Månedens chatter", "name": "Twitch chatten", "text": "Den der holder energien oppe.", "icon": "💬", "active": true, "priority": 1}, {"title": "Top requester", "name": "Musikønsker", "text": "Den der finder de bedste tracks.", "icon": "🎧", "active": true, "priority": 2}, {"title": "Community hero", "name": "DJ FOLSOE Family", "text": "Kærlighed til dem der støtter streamen.", "icon": "💜", "active": true, "priority": 3}]),liveRequestWall:ceItems(core,"liveRequestWall",[{"user": "Chat", "song": "Skriv !ønske Artist - Title", "language": "da", "time": "live", "active": true, "priority": 1}, {"user": "Chat", "song": "Use !request Artist - Title", "language": "en", "time": "live", "active": true, "priority": 2}, {"user": "Chat", "song": "Nutze !Wunsch Künstler - Titel", "language": "de", "time": "live", "active": true, "priority": 3}]),musicDiscovery:ceItems(core,"musicDiscovery",[{"artist": "Mau P", "title": "The Less I Know The Better", "genre": "Dance", "note": "Ny energi til chart-showet.", "active": true, "priority": 1}, {"artist": "Peggy Gou", "title": "Find The Way", "genre": "House", "note": "Frisk house-vibe til streamen.", "active": true, "priority": 2}, {"artist": "Anyma", "title": "Hypnotized", "genre": "Melodic Techno", "note": "Kan blive en stærk bobler.", "active": true, "priority": 3}]),showArchive:ceItems(core,"showArchive",[{"title": "Trance Tuesday Highlights", "date": "Seneste show", "text": "Melodisk trance, energi og community moments.", "image": "", "active": true, "priority": 1}, {"title": "Fredagsbar Replay", "date": "Seneste fredag", "text": "Weekendstemning og live DJ-energi.", "image": "", "active": true, "priority": 2}, {"title": "Top 20 Countdown", "date": "Denne uge", "text": "Ugens vigtigste tracks og discoveries.", "image": "", "active": true, "priority": 3}])});
      if (path === "/api/tv-guide") { if (request.method === "GET") return json({ok:true,items:ceItems(core,"tvGuide",[{"day": "Tirsdag", "time": "18:30", "title": "Trance Tuesday", "text": "Store melodier, energi og trance-fællesskab.", "type": "trance", "active": true, "priority": 1}, {"day": "Torsdag", "time": "18:30", "title": "FOLSOE Top 20", "text": "Ugens største tracks i countdown-format.", "type": "chart", "active": true, "priority": 2}, {"day": "Fredag", "time": "20:00", "title": "Fredagsbar", "text": "Live DJ med sjov, ballade og weekendstemning.", "type": "party", "active": true, "priority": 3}, {"day": "Søndag", "time": "20:00", "title": "Retro Hits", "text": "Klassikere, nostalgi og gamle hits med nyt liv.", "type": "retro", "active": true, "priority": 4}])}); return saveCEList(request, env, core, "tvGuide"); }
      if (path === "/api/viewer-journey") { if (request.method === "GET") return json({ok:true,items:ceItems(core,"viewerJourney",[{"key": "followers", "label": "Followers", "current": 870, "target": 1000, "text": "Rejsen mod 1000 followers på Twitch.", "active": true, "priority": 1}, {"key": "subs", "label": "Subs", "current": 0, "target": 100, "text": "Subs hjælper med teknik, grafik og shows.", "active": true, "priority": 2}, {"key": "community", "label": "Community", "current": 0, "target": 500, "text": "Flere aktive seere og mere fællesskab.", "active": true, "priority": 3}])}); return saveCEList(request, env, core, "viewerJourney"); }
      if (path === "/api/hall-of-fame") { if (request.method === "GET") return json({ok:true,items:ceItems(core,"hallOfFame",[{"title": "Månedens chatter", "name": "Twitch chatten", "text": "Den der holder energien oppe.", "icon": "💬", "active": true, "priority": 1}, {"title": "Top requester", "name": "Musikønsker", "text": "Den der finder de bedste tracks.", "icon": "🎧", "active": true, "priority": 2}, {"title": "Community hero", "name": "DJ FOLSOE Family", "text": "Kærlighed til dem der støtter streamen.", "icon": "💜", "active": true, "priority": 3}])}); return saveCEList(request, env, core, "hallOfFame"); }
      if (path === "/api/live-request-wall") { if (request.method === "GET") return json({ok:true,items:ceItems(core,"liveRequestWall",[{"user": "Chat", "song": "Skriv !ønske Artist - Title", "language": "da", "time": "live", "active": true, "priority": 1}, {"user": "Chat", "song": "Use !request Artist - Title", "language": "en", "time": "live", "active": true, "priority": 2}, {"user": "Chat", "song": "Nutze !Wunsch Künstler - Titel", "language": "de", "time": "live", "active": true, "priority": 3}])}); return saveCEList(request, env, core, "liveRequestWall"); }
      if (path === "/api/music-discovery") { if (request.method === "GET") return json({ok:true,items:ceItems(core,"musicDiscovery",[{"artist": "Mau P", "title": "The Less I Know The Better", "genre": "Dance", "note": "Ny energi til chart-showet.", "active": true, "priority": 1}, {"artist": "Peggy Gou", "title": "Find The Way", "genre": "House", "note": "Frisk house-vibe til streamen.", "active": true, "priority": 2}, {"artist": "Anyma", "title": "Hypnotized", "genre": "Melodic Techno", "note": "Kan blive en stærk bobler.", "active": true, "priority": 3}])}); return saveCEList(request, env, core, "musicDiscovery"); }
      if (path === "/api/show-archive") { if (request.method === "GET") return json({ok:true,items:ceItems(core,"showArchive",[{"title": "Trance Tuesday Highlights", "date": "Seneste show", "text": "Melodisk trance, energi og community moments.", "image": "", "active": true, "priority": 1}, {"title": "Fredagsbar Replay", "date": "Seneste fredag", "text": "Weekendstemning og live DJ-energi.", "image": "", "active": true, "priority": 2}, {"title": "Top 20 Countdown", "date": "Denne uge", "text": "Ugens vigtigste tracks og discoveries.", "image": "", "active": true, "priority": 3}])}); return saveCEList(request, env, core, "showArchive"); }
      if (path === "/api/requests") {
        if (request.method === "POST") return addRequest(request, env, core);
        if (request.method === "PUT") return updateRequests(request, env, core);
        return json(await recentRequestsPayload(env, core));
      }
      if (path === "/api/request-stats") {
        const list = await getRequestList(env, core);
        return json({ok:true,stats:requestStats(list)});
      }
      if (path === "/api/top20") {
        if (request.method === "GET") return json({ok:true,items:(core.top20&&core.top20.length?core.top20:DEFAULT_CORE.top20||[]).slice(0,20)});
        return saveTop20Stable(request, env, core);
      }
      if (path === "/api/discovery-picks") {
        if (request.method === "GET") return json({ok:true,items:(core.discoveryPicks&&core.discoveryPicks.length?core.discoveryPicks:DEFAULT_CORE.discoveryPicks||[]).slice(0,3)});
        return saveDiscoveryPicksStable(request, env, core);
      }
      if (path === "/api/seo") {
        if (request.method === "GET") return json(buildSeoPayload(core, url.searchParams.get("lang") || core.language || "da"));
        if (!adminOk(request,env)) return json({error:"Unauthorized"},401);
        const body = await request.json();
        core.seo = body.seo || body.item || body;
        await putCore(env,core);
        return json({ok:true,seo:core.seo});
      }
      if (path === "/sitemap.xml") return new Response(sitemapXml(core), {headers:{"content-type":"application/xml; charset=utf-8",...cors()}});
      if (path === "/robots.txt") return new Response(robotsTxt(core), {headers:{"content-type":"text/plain; charset=utf-8",...cors()}});
      if (path === "/api/mod-team") {
        if (request.method === "GET") return json({ok:true,items:await enrichMods(env, core)});
        return saveList(request,env,core,"modTeam");
      }
      if (path === "/api/community-wall") {
        if (request.method === "GET") return json({ok:true,items:core.communityWall||DEFAULT_CORE.communityWall||[]});
        return saveList(request,env,core,"communityWall");
      }
      ...
      if (path === "/api/overlay-content") {
        ...
        if (!adminOk(request,env)) return json({error:"Unauthorized"},401);
        const body = await request.json();
        core.overlayContent = body.items || body.overlayContent || {};
        await putCore(env,core);
        return json({ok:true,items:core.overlayContent});
      }
